...
class TestWeatherEndpoints(TestCase):
    ...
    @mock.patch(
        'backend.routers.weather.cache.location',
        return_value=fixtures.CACHE_LOCATION
    )
    def test_widget_overview(self, location_mock, uvi_forecast_mock, air_pollution_mock, one_call_mock):
            result = asyncio.run(
                weather.widget_overview(lat=0.0, lon=0.0)
            )
            self.assertDictEqual(
                result,
                router_fixtures.WIDGET_OVERVIEW_RESPONSE
            )
