...
def weather_points(
    data: List[models.WeatherDataBaseModel],
    munge: Callable
):
    for each in data:
...
def munge_weather_data(data: models.WeatherDataBaseModel) -> tuple:
    ...
    """
    # get name from model
    measurement = data.__name__.lower()
    ...
    retval = {
        f'{measurement}_{field}': data.pop(field)
        for field in flatten_keys
    }
    # check for weather
...
async def weather_data():
    ...
    current_points = weather_points(
        data=[data.current],
        munge=munge_weather_data
    )
    hourly_points = weather_points(
        data=data.hourly,
        munge=munge_weather_data
    )
    minutely_points = weather_points(
        data=data.minutely,
        munge=munge_weather_data
    )
    daily_points = weather_points(
        data=data.minutely,
        munge=munge_weather_data
    )
    alert_points = weather_points(
        data=data.alerts,
        munge=munge_alert_data
    )
    for points in [current_points, hourly_points, minutely_points, daily_points, alert_points]:
